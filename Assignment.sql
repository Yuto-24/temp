...
FROM URIAGE_M
RIGHT OUTER JOIN SHOHIN_M ON URIAGE_M.UM_SCODE = SHOHIN_M.S_CODE
...
FROM URIAGE_M
INNER JOIN SHOHIN_M ON URIAGE_M.UM_SCODE = SHOHIN_M.S_CODE
...
FROM SHOHIN_M
LEFT OUTER JOIN URIAGE_M ON URIAGE_M.UM_SCODE = SHOHIN_M.S_CODE
...
FROM URIAGE_H
INNER JOIN URIAGE_M ON URIAGE_H.UH_DENNO = URIAGE_M.UM_DENNO
INNER JOIN TOKUI_M ON TOKUI_M.T_CODE = URIAGE_H.UH_TCODE
INNER JOIN SHOHIN_M ON URIAGE_M.UM_SCODE = SHOHIN_M.S_CODE
...
FROM TOKUI_M
--WHERE T_CODE = 1001
INNER JOIN URIAGE_H ON T_CODE = UH_TCODE
INNER JOIN URIAGE_M ON UH_DENNO = UM_DENNO
LEFT OUTER JOIN SHOHIN_M ON UM_SCODE = S_CODE
...
WHERE T_CODE not in (
	SELECT T_CODE
	FROM URIAGE_H
	LEFT OUTER JOIN TOKUI_M ON T_CODE = UH_TCODE
	LEFT OUTER JOIN URIAGE_M ON UH_DENNO = UM_DENNO
	WHERE UM_SCODE = 1001
	GROUP BY T_CODE
)
;
...
(SHOHIN_W.S_CODE, SHOHIN_W.S_NAME)
SELECT SHOHIN_M.S_CODE, SHOHIN_M.S_NAME
FROM SHOHIN_M
WHERE S_CODE = 5001
;
...
FROM SHOHIN_M
LEFT OUTER JOIN URIAGE_M ON UM_SCODE = S_CODE
LEFT OUTER JOIN URIAGE_H ON UM_DENNO = UH_DENNO
LEFT OUTER JOIN TOKUI_M ON UH_TCODE = T_CODE
WHERE T_CODE = 1001
...
SELECT S_CODE--, S_NAME
	FROM SHOHIN_M
	INNER JOIN URIAGE_M ON UM_SCODE = S_CODE
	INNER JOIN URIAGE_H ON UM_DENNO = UH_DENNO
	INNER JOIN TOKUI_M ON UH_TCODE = T_CODE
	WHERE T_CODE = 1001
	GROUP BY S_CODE, S_NAME
)
;
